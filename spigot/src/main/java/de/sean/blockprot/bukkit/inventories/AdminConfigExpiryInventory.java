...
/**
 * Expiry category of the inventory-based admin config editor.
 */
public final class AdminConfigExpiryInventory extends BlockProtInventory {
    ...
    public AdminConfigExpiryInventory() { super(false); }

    @Override int getSize() { return InventoryConstants.sextupletLine; }

    @Override
    String getTranslatedInventoryName() {
        return Translator.get(TranslationKey.DIALOGS__ADMIN_CONFIG__CAT_EXPIRY);
    }
    ...
    public void onClick(@NotNull InventoryClickEvent event, @NotNull InventoryState state) {
        ...
        if (slot == SLOT_BACK) {
            ...
            cfg.setWorldExpiryEnabled(!cfg.isWorldExpiryEnabled());
            player.openInventory(fill(player));
        } else if (slot == SLOT_CHECK_INTERVAL) {
            TextInput.open(player, BlockProt.getInstance(),
                Translator.get(TranslationKey.DIALOGS__ADMIN_CONFIG__EXPIRY__CHECK_INTERVAL_HINT), input -> {
                    if (input == null || input.isBlank()) return;
                    try {
                        cfg.setAndSave("world_expiry.check_interval_minutes", Integer.parseInt(input.trim()));
