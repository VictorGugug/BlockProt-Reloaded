...
public final class ReloadCoordinator {
    ...
    private static synchronized ReloadReport executeReload(@NotNull ReloadReport.ReloadSource source, @Nullable UUID actor) {
        ...
        Map<String, Object> beforeSnapshot = captureFullSnapshot(plugin);
        ...
        try {
            ...
            Map<String, Object> afterSnapshot = captureFullSnapshot(plugin);
    ...
    private static Map<String, Object> captureFullSnapshot(@NotNull BlockProt plugin) {
    ...
    private static void logReport(@NotNull ReloadReport report, @Nullable UUID actor) {
        ...
        if (report.getDiffs().isEmpty()) {
            BlockProtLogger.log("reload", "Reload completed: no configuration values changed.");
        } else {
            for (ReloadReport.ChangeDiff diff : report.getDiffs()) {
                BlockProtLogger.log("reload", "  " + diff.toString());
            }
            BlockProtLogger.log("reload", "Reload completed: " + report.getDiffs().size() + " change(s) activated.");
        }
