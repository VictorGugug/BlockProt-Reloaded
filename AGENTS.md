...
- **Standardized commit format**: Every commit must follow the project commit convention `BPR:<branch>(<type>): <summary>` (for example, `BPR:main(fix): Resolve duplicate interact event on off-hand click` or `BPR:main(add): Support Paper Dialog menus`). `BPR` identifies BlockProt Reloaded, `:<branch>` indicates the destination branch (for example `:main` or future branches), and `(<type>)` specifies the change type (`fix`, `add`, `feat`, `refactor`, `docs`, `chore`). Commits must be title-only without any commit body or multi-line description.
- **Pull request formatting**: When submitting a pull request, provide both a standardized title matching the commit format and a thorough description detailing the problem, technical implementation, and verification results.
