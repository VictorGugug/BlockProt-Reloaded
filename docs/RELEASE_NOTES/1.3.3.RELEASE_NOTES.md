![1.3.3](https://raw.githubusercontent.com/VictorGugug/BlockProt-Reloaded/main/images/RELEASE%20TITLES/RELEASES%20NOTES/RELEASE_NOTES_1.3.3.png)
...
### 1.3.4 - dialog system (released)

The dialog system proposed in [issue #1](https://github.com/VictorGugug/BlockProt-Reloaded/issues/1) shipped with 1.3.4. See [the 1.3.4 release notes](1.3.4.RELEASE_NOTES.md) for full details on all dialog screens, the admin config editor, language management, and integration changes.

### 1.3.5 - language snapshots

Starting in 1.3.5, each new language contributed by the community will be distributed as an independent snapshot resource rather than bundled into the plugin JAR. Languages can be added, updated, or removed without waiting for a full plugin release. Community translators maintain their own language files independent of the release cycle. See the 1.3.4 release notes for details.
...
Requested in [issue #298](https://github.com/spnda/BlockProt/issues/298).
...
| [#238](https://github.com/spnda/BlockProt/issues/238) | Piston Protection Shulker Box | Shulker box check added to piston retract logic |
...
| [#349](https://github.com/spnda/BlockProt/issues/349) | Warn spam in console (NBT swap error) | StatHandler rewrite with temp-write-validate-replace |
| [#344](https://github.com/spnda/BlockProt/issues/344) | Console spam (NbtApiException) | Same NBT swap warning as #349, covered by the same StatHandler rewrite |

### Features open upstream, implemented in fork

| Issue | Title | Fork Implementation |
|---|---|---|
| [#298](https://github.com/spnda/BlockProt/issues/298) | Integration support for ClaimChunk | `ClaimChunkIntegration`, see "ClaimChunk integration" above |
| [#295](https://github.com/spnda/BlockProt/issues/295) | Lock trapdoors and iron doors | Trapdoors are members of the `TRAPDOOR` sub-family under `lockable_blocks`; all `*_DOOR` materials, including `IRON_DOOR`, are members of the `DOORS` family under `lockable_doors` |
| [#318](https://github.com/spnda/BlockProt/issues/318) | Ability to configure lockable blocks per world | `per_worlds_config` in `config.yml` plus `worlds.yml`, see "Per-world lockable GUI" above |
| [#303](https://github.com/spnda/BlockProt/issues/303) | Respect spawn protection radius | `respect_spawn_protection` in `config.yml` (default `true`); non-op, non-admin players cannot lock a block inside the server's spawn-protection radius |
| [#282](https://github.com/spnda/BlockProt/issues/282) | MySQL Support | `HybridDatabase`, a Hikari-pooled MySQL/MariaDB index configured in `mysql/mysql.yml`. NBT stays the source of truth for ownership and friends; the database mirrors block locations and global trust lists for auditing and search on large servers |
| [#334](https://github.com/spnda/BlockProt/issues/334) | Colored prompt messages | Fully supported via both MiniMessage and legacy color codes in Translator |
| [#192](https://github.com/spnda/BlockProt/issues/192) | Support Inspect contents block name i18n | Inspect contents inventory titles are now localized |
| [#346](https://github.com/spnda/BlockProt/issues/346) | New configuration option | Added `clear_protection_on_shulker_break` config |
| [#322](https://github.com/spnda/BlockProt/issues/322) | Toggle for simplified hopper logic | Added `simplified_hopper_logic` config |
| [#269](https://github.com/spnda/BlockProt/issues/269) | Shows the owner of the box | Implemented via Action Bar on interact |
| [#267](https://github.com/spnda/BlockProt/issues/267) | Expire locks after owner offline for x time | Implemented via world expiry subsystem |
| [#228](https://github.com/spnda/BlockProt/issues/228) | Separate Redstone in config | Implemented via `redstone_disallowed_by_default` |
| [#225](https://github.com/spnda/BlockProt/issues/225) | Stats more Features | Covered by the `StatisticListInventory` GUI |
| [#183](https://github.com/spnda/BlockProt/issues/183) | Support Residence | `ResidenceIntegration` added |
| [#154](https://github.com/spnda/BlockProt/issues/154) | Support for GriefPrevention | `GriefPreventionIntegration` added |
