...
- `paper-plugin.yml` added next to `plugin.yml`: the JAR is now also a Paper plugin, so Paper, Purpur and Folia load it through the Paper plugin loader, and Modrinth's loader auto-detection now finds Paper (and Folia via `folia-supported`) instead of only Folia when a build is uploaded. `/blockprot` is registered programmatically when the descriptor provides no commands (`BlockProt.createPluginCommand`); behavior on Spigot/Bukkit is unchanged. Modrinth cannot auto-detect Bukkit/Spigot/Purpur for any Bukkit plugin, so those loaders are still selected manually at upload.
...
- `spigot/src/main/resources/paper-plugin.yml`: Paper plugin descriptor mirroring `plugin.yml` (no `commands` section; Paper registers descriptor commands via its own system, so `/blockprot` is registered in code instead)
...
- `spigot/src/main/java/de/sean/blockprot/bukkit/BlockProt.java`: dialog init; lang cleanup; logger init; `/blockprot` command registered programmatically for the Paper plugin loader (`createPluginCommand`)
