...
- `gradle.properties` version set to `1.3.4` with `versionSuffix` set to `BDev` (beta development build). This suffix enables publishing builds tagged as Beta on distribution platforms like Modrinth and CurseForge while active development on 1.3.4 continues. BPR versioning and release strategies will be further explored in upcoming builds.
...
Verified against `git diff --stat a8c7c867^..HEAD` (24 commits, `84f2bed` through `73cf648`, the full 1.3.4 development range following the 1.3.3 release commit `a8c7c867`). 128 files touched. Files that were unchanged in this range have been removed from the lists below even where an earlier draft of these notes attributed 1.3.4 work to them.
...
- `gradle.properties`: version set to 1.3.4 with `versionSuffix: BDev` (beta development tag for Modrinth/CurseForge)
